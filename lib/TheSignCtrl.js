...
const { PasswordWrongError, SigninFailedError } = require('./Errors')
...
class TheSignCtrl extends TheCtrl {
  ...
  async signin (name, password, options = {}) {
    ...
    if (!valid) {
      throw new SigninFailedError('Signin failed!')
    }
    await user.sync()
  ...
  async signdel () {
    ...
    await s.assertSigned()
    let user = await s.getSigned()
    delete s.session[ USER_SESSION_KEY ]

    let { entities: signs } = await UserSign.list({ filter: { user } })
    await UserSign.destroyBulk(signs.map(({ id }) => id))
